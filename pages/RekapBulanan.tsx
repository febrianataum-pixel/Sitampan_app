  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800 dark:text-slate-100 uppercase tracking-tighter">Rekap Tahunan</h2>
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <button onClick={handleExportCSV} className="flex-1 sm:flex-none bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 font-black shadow-xl hover:bg-black transition-all active:scale-95 text-[10px] uppercase tracking-widest">
            <Download size={16}/> CSV
          </button>
        </div>
      </div>

      <div className="bg-white dark:bg-surface-dark p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-sm flex items-center gap-4 theme-transition">
        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl">
          <Calendar size={24} />
        </div>
        <div className="flex-1">
          <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 mb-1 uppercase tracking-widest">Pilih Tahun Laporan</label>
          <input 
            type="number" 
            className="w-full max-w-[200px] bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 px-4 py-2 rounded-xl font-black text-slate-700 dark:text-slate-200 outline-none focus:ring-4 focus:ring-blue-50 dark:focus:ring-blue-900/20 transition-all text-lg" 
            value={selectedYear} 
      ...
      <div className="bg-white dark:bg-surface-dark rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-white/5 overflow-hidden theme-transition">
        <div className="overflow-x-auto scrollbar-hide">
          <table className="w-full text-left text-[11px] min-w-[1200px]">
            <thead className="bg-slate-50/50 dark:bg-white/5 font-black border-b dark:border-white/5 text-slate-400 dark:text-slate-500 uppercase tracking-widest">
              <tr>
                <th className="px-6 py-5 sticky left-0 bg-slate-50 dark:bg-surface-dark z-10 shadow-[2px_0_5px_rgba(0,0,0,0.05)]">Nama Barang</th>
                {MONTHS.map(m => (
                  <th key={m} className="px-2 py-5 text-center w-16">{m.substring(0, 3)}</th>
                ))}
                <th className="px-6 py-5 text-center bg-blue-50/50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400">Total</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {rekapTahunan.length > 0 ? rekapTahunan.map((r, idx) => (
                <tr key={idx} className="hover:bg-blue-50/20 dark:hover:bg-blue-900/10 transition-colors group">
                  <td className="px-6 py-4 sticky left-0 bg-white dark:bg-surface-dark group-hover:bg-blue-50/20 dark:group-hover:bg-blue-900/10 z-10 shadow-[2px_0_5px_rgba(0,0,0,0.05)]">
                    <p className="font-bold text-slate-800 dark:text-slate-200">{r.namaBarang}</p>
                    <p className="text-[9px] text-slate-400 dark:text-slate-500 font-mono">{r.kodeBarang}</p>
                  </td>
                  {r.monthlyTotals.map((val, mIdx) => (
                    <td key={mIdx} className={`px-2 py-4 text-center font-bold ${val > 0 ? 'text-slate-700 dark:text-slate-300' : 'text-slate-200 dark:text-slate-800 font-normal'}`}>
                      {val || '-'}
                    </td>
                  ))}
                  <td className="px-6 py-4 text-center font-black text-blue-600 dark:text-blue-400 bg-blue-50/30 dark:bg-blue-900/20">
                    {r.totalYear} <span className="text-[8px] text-slate-400 dark:text-slate-500 font-normal ml-1">{r.satuan}</span>
                  </td>
                </tr>
              )) : (
      ...
      <div className="bg-blue-50 dark:bg-blue-900/10 p-6 rounded-[2rem] border border-blue-100 dark:border-blue-900/20 text-blue-700 dark:text-blue-400 flex items-start gap-4">
        <div className="p-2 bg-blue-600 dark:bg-blue-500 text-white rounded-lg shrink-0 mt-1">
          <Calendar size={16}/>
        </div>
